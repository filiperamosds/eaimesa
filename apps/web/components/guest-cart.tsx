...
export function GuestCart({ cart, onChange, canOrder }: Props) {
  const [open, setOpen] = useState(false);
  const [orderNote, setOrderNote] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);
  const [sent, setSent] = useState<GuestOrder | null>(null);
    ...
    <>
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-card/95 px-4 py-3 backdrop-blur-xl">
        <div className="mx-auto flex max-w-lg items-center justify-between gap-3">
          {sent && count === 0 ? (
            <p className="text-sm text-sage">Pedido enviado · {formatBrlFromCents(sent.totalCents)}</p>
          ) : (
            <p className="text-sm">
              <span className="font-medium">{count} {count === 1 ? "item" : "itens"}</span>
          ...
          <div className="surface flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden p-5">
            ...
            {sent ? (
              <p className="mt-3 text-sm text-sage">
                Último envio: {sent.items.map((i) => `${i.qty}× ${i.name}`).join(" · ")} ·{" "}
                {formatBrlFromCents(sent.totalCents)}
              </p>
            ) : null}
            {cart.length === 0 ? (
              <p className="mt-6 text-sm text-ink-soft">Cesta vazia. Adicione itens no cardápio.</p>
            ) : (
